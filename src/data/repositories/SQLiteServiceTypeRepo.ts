...
export class SQLiteServiceTypeRepo implements IServiceTypeRepo {
  ...
  async getAll(): Promise<ServiceType[]> {
    const rows = await this.db.getAllAsync<ServiceTypeRow>(
      'SELECT * FROM service_types ORDER BY sort_order ASC, name ASC;',
    );
  ...
  async insert(input: CreateServiceTypeInput): Promise<ServiceType> {
    ...
    await this.db.runAsync(
      `INSERT INTO service_types (id, name, icon, is_system, sort_order, created_at)
       VALUES (?, ?, ?, 0, ?, ?);`,
      [id, input.name, input.icon, input.sortOrder, now],
    );
    return { id, name: input.name, icon: input.icon, isSystem: false, sortOrder: input.sortOrder, createdAt: now };
  }

  async update(id: string, name: string, icon: string): Promise<void> {
    await this.db.runAsync(
      'UPDATE service_types SET name = ?, icon = ? WHERE id = ? AND is_system = 0;',
      [name, icon, id],
    );
  }
  ...
  async delete(id: string): Promise<void> {
    await this.db.runAsync(
      'DELETE FROM service_types WHERE id = ? AND is_system = 0;',
      [id],
    );
  }
}
